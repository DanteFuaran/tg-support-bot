...
# 3️⃣ Подготовка каталога
(mkdir -p "$INSTALL_DIR" >/dev/null 2>&1) & show_spinner "Подготовка каталога"

# Проверка на старые файлы
if [ -d "$INSTALL_DIR" ] && [ "$(ls -A "$INSTALL_DIR" 2>/dev/null || true)" ]; then
  ...
  case "$confirm" in
    [yY][eE][sS]|[yY])
      (sleep 1 && rm -rf "$INSTALL_DIR" >/dev/null 2>&1) & show_spinner "Удаление старых файлов"
      mkdir -p "$INSTALL_DIR"
      cd "$INSTALL_DIR"
      (git clone https://github.com/DanteFuaran/tg-support-bot.git . >/dev/null 2>&1) & show_spinner "Клонирование репозитория"
      ;;
    *)
...
(python3 -m venv .venv >/dev/null 2>&1) & show_spinner "Создание виртуального окружения"
source .venv/bin/activate
(pip install -r requirements.txt >/dev/null 2>&1) & show_spinner "Установка зависимостей Python"
deactivate

echo -e "${GREEN}✅${NC} Все зависимости установлены\n"

# 4️⃣ Конфиг и хранилище
(mkdir -p bot && cat > bot/config.py << 'EOF'
import os, sys
...
(cp storage.example.json storage.json 2>/dev/null || touch storage.json) & show_spinner "Создание базы данных"
...
GREEN='\033[0;32m'; RED='\033[0;31m'; BLUE='\033[1;34m'; WHITE='\033[1;37m'; NC='\033[0m'
...
find "$INSTALL_DIR" -type d -name ".git" -exec rm -rf {} + >/dev/null 2>&1
rm -f "$INSTALL_DIR/.gitignore" \
      ...
      "$INSTALL_DIR/storage.example.json" >/dev/null 2>&1
