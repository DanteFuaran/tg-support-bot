...
echo
echo -e "🔐 Для продолжения требуется лицензионный ключ (ТЕСТ 4)"
...
validate_license_key() {
    ...
    echo -e "${YELLOW}🔍 Отладка: Ключ = '$input_key'${NC}"
    echo -e "${YELLOW}🔍 Отладка: Хеш  = '$key_hash'${NC}"
    
    # Получаем содержимое файла с хешами
    local remote_hashes
    ...
    echo -e "${YELLOW}🔍 Отладка: Загружено хешей: $(echo "$remote_hashes" | wc -l)${NC}"
    echo -e "${YELLOW}🔍 Отладка: Первый хеш в файле: $(echo "$remote_hashes" | head -1)${NC}"
    
    # Проверяем наличие хеша в онлайн-файле
    if echo "$remote_hashes" | grep -q "^$key_hash$"; then
        echo -e "${GREEN}✅ Отладка: Хеш найден в файле!${NC}"
        return 0
    else
        echo -e "${RED}❌ Отладка: Хеш НЕ найден в файле${NC}"
        return 1
    fi
}
